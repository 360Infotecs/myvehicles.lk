...
function create_company()
{
    ...
    else if ($imagetype == "image/jpg" || $imagetype == 'image/jpeg' || $imagetype == 'image/png' || $imagetype == 'image/gif')
    {
        ...
        if ($phoneNo)
        {
            ...
            {
                return Insert_Company($agentId, $companyname, $phoneNo);
            }
        }
    }
    return false;
...
function edit_company()
{
    ...
    $cn       = $_POST['username'];
    $companyname = validate_empty($cn, 'User Name');
    if ($companyname !== false)
    {
        $tpNo      = $_POST['phoneno'];
        $phoneNo   = validate_empty($tpNo, 'Phone No');
        ...
        /*    if (!company_exist($phoneNo))
            {*/
                return Update_User($user_id,$agentId, $companyname, $phoneNo);
            /*}*/
        }
    }
    return false;
...
function Insert_Company($agentId, $companyname, $phoneNo)
{
	global $pdo, $msg;
	$statement = $pdo->prepare("
			INSERT INTO company(Id, CompanyId, AgentId, CompanyName, AddressLine1, 
			AddressLine2, AddressLine3, ContactPerson, PhoneNo, Mobile, Email, 
...
function Update_User($user_id, $agentId, $companyname, $phoneNo)
{
	global $pdo, $msg;
